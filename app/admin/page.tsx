...
export default function AdminPage() {
  ...
  return <main className="admin-shell"><aside className="admin-sidebar"><a className="brand light" href="/"><img className="brand-logo" src="/Logo%20(1)/iskcon_logo.svg" alt="ISKCON logo"/><span><b>ISKCON</b><small>SAHARANPUR</small></span></a><nav><a href="#enquiries"><Inbox/> Enquiries</a><a href="#page-content"><Save/> Page content</a><a href="#darshan"><Clock3/> Darshan timings</a><a href="#events"><CalendarDays/> Upcoming events</a></nav><button onClick={logout}><LogOut/> Sign out</button></aside><section className="admin-main"><header><div><p className="eyebrow dark">Temple developer area</p><h1>Content dashboard</h1></div><span className="status-dot">Connected</span></header>{message&&<div role="status" aria-live="polite" className={`admin-notice ${/could not|offline|please choose|invalid/i.test(message)?'notice-error':'notice-success'}`}>{message}<button onClick={()=>setMessage('')} aria-label="Dismiss notification">×</button></div>}
