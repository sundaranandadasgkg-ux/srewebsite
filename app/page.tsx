...
export default function Home() {
  ...
  return <main>
    ...
    <section className={`temple-gallery ${gallery.length?'has-photos':'awaiting-photos'}`} id="discover"><div className="gallery-heading"><p className="eyebrow">Discover our temple</p><div className="quote-mark">“</div><blockquote>No servant in this world<br/><em>is more dear to Me.</em></blockquote><p className="quote-translation">Service offered with devotion brings the soul closer to Krishna.</p><span className="quote-credit">— Bhagavad-gita 18.69</span></div>{gallery.length>0&&<div className="gallery-stage"><button className="gallery-photo" onClick={()=>setSelectedGalleryImage(gallery[slide])} aria-label="Enlarge temple photograph"><img key={gallery[slide].id} src={gallery[slide].image} alt={gallery[slide].alt}/><span>View photograph</span></button>{gallery.length>1&&<><button className="gallery-arrow previous" onClick={()=>setSlide((slide-1+gallery.length)%gallery.length)} aria-label="Previous photo"><ArrowLeft/></button><button className="gallery-arrow next" onClick={()=>setSlide((slide+1)%gallery.length)} aria-label="Next photo"><ArrowRight/></button><div className="gallery-counter"><b>{String(slide+1).padStart(2,'0')}</b><i/><span>{String(gallery.length).padStart(2,'0')}</span></div></>}</div>}</section>
