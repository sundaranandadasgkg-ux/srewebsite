...
import { ArrowRight, BookOpen, Clock3, Heart, MapPin, Menu, Music2, Phone, X } from 'lucide-react';
...
export default function Home() {
  ...
  const [events, setEvents] = useState<Array<{_id:string;title:string;date:string;description:string}>>([]);
  useEffect(() => { fetch(`${apiUrl}/api/content`).then(r=>r.json()).then(data=>{ if(data.schedule?.length) setSchedule(data.schedule); if(data.events) setEvents(data.events); if(data.sections) setContent(current=>({...current,...data.sections})); }).catch(()=>{}); }, []);
  ...
  return <main>
    ...
    <section className="welcome" id="about"><div><p className="eyebrow dark">{content.about.eyebrow}</p><h2>{content.about.title}</h2></div><div className="welcome-copy"><p>{content.about.body}</p><a href="#visit">Discover our temple <ArrowRight size={17}/></a></div></section>
    ...
    {events.length>0&&<section className="events-section"><div className="section-heading"><div><p className="eyebrow dark">Temple calendar</p><h2>Upcoming events.</h2></div><p>Come celebrate, chant and serve with the ISKCON Saharanpur community.</p></div><div className="event-grid">{events.map(event=><article className="event-card" key={event._id}><time>{new Date(event.date).toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'})}</time><h3>{event.title}</h3><p>{event.description}</p></article>)}</div></section>}
    <section className="visit" id="visit"><div className="visit-card"><MapPin/><p className="eyebrow dark">{content.visit.eyebrow}</p><h2>{content.visit.title}</h2><p className="preline">{content.visit.address}</p><div className="contact-links"><a href="https://maps.google.com/?q=ISKCON+Saharanpur" target="_blank">Get directions <ArrowRight size={16}/></a><a href={`tel:${content.visit.phone.replace(/\s/g,'')}`}>{content.visit.phone}</a></div></div><div className="visit-image" role="img" aria-label="Joyful ISKCON kirtan gathering"/></section>
